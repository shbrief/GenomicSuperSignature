...
#' Barplot of GSEA output
#'
#' @import clusterProfiler.dplyr
#'
#' @param PCAmodel PCAGenomicSignature object.
#' @param ind An interger. Index of avgLoading to apply GSEA.
#' @param category A character. MSigDB category. Options are "H","C1","C2"(default),
#' "C3","C4","C5","C6", and "C7"
...
msigdb_gsea <- function(ind, PCAmodel, category = "C2", n = 10) {
    ...
    ## Barplot
    y <- mutate(ms, ordering = abs(NES)) %>% arrange(desc(ordering))
    y_bar <- group_by(y, sign(NES)) %>% dplyr::slice(1:n)
