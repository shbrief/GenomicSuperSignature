...
#'
#' @return A data frame with \code{n} rows and 4 columns; Description, NES, pvalue, and qvalues
#'
#' @examples
#' data(miniRAVmodel)
...
annotateRAV <- function(RAVmodel, ind, n = 5, abs = FALSE) {
  ...
  # absolute value of NES if it's specified
  FUN <- if (abs) {abs} else {I}

  # subset GSEA results
  topAnnotation <- annotatedCluster[order(FUN(annotatedCluster$NES), decreasing = TRUE),,drop = FALSE][seq_len(n),]
