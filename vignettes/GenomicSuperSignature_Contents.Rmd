...
* model(RAVmodel) : avgLoading matrix   
...
database. You can access it through `GenomicSuperSignature::model` (equivalent of `SummarizedExperiment::assay`). 
Rows are genes and columns are RAVs.

Here, RAVmodel consists of 13,934 genes and 4,764 RAVs.

```{r}
class(model(RAVmodel))
dim(model(RAVmodel))
model(RAVmodel)[1:4, 1:4]
```
