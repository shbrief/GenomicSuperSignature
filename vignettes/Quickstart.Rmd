...
`GenomicSuperSignature::getModel` function. This model is built from top 20 PCs 
of 536 studies (containing 44,890 samples) containing 13,934 common genes from 
each of 536 study's top 90% varying genes based on their study-level standard 
deviation. There are two versions of this using different gene sets for 
GSEA-based annotation; MSigDB C2 (`C2`) and three priors from PLIER package 
(`PLIERpriors`). In this vignette, we are using the `C2` annotated model.
...
## Example dataset
**Human B-cell expression dataset** The human B-cell dataset (Gene Expression 
Omnibus series GSE2350) consists of 211 normal and tumor human B-cell phenotypes
whose expression was profiled on Affymatrix HG-U95Av2 arrays, and it is 
contained in an ExpressionSet object with 6,249 features x 211 samples.
...
matrix, ExpressionSet, or SummarizedExperiment. Just make sure that genes in 
rows are in a 'symbol' format. 


# Validate
## HeatmapTable
`heatmapTable` outputs a two panel table: top panel represents average 
silhouette width (avg.sw) and the bottom panel represents the validation score.

You can display the validation output in multiple ways. For example, if you 
specify `scoreCutoff` argument of `heatmapTable`, any validation result above 
that score will be shown. If you specify the number of top validation results 
(= n) through `num.out` argument of `heatmapTable`, the output will be a 
n-columned heatmap table. You can also use the average silhouette width 
(`swCutoff`), the size of cluster (`clsizecutoff`), PC from dataset (`whichPC`).
...
single-element clusters in one graph, where x-axis represent average silhouette 
...
- **score** : the top validation score between 8 PCs of the dataset and the 
cluster   
...
your dataset. Based on the heatmap table above, 1st-3rd RAVs (2538, 1139, and 
884) show high validation scores with positive average silhouette widths, so we 
draw wordclouds of those RAVs using `drawWordcloud` function. You need to 
provide RAVmodel and the index of the RAV you are interested in.
...
Because the test dataset is human B-cell expression data, we tried the model 
annotated with blood-associated gene sets. 

```{r message=FALSE, warning=FALSE}
RAVmodel <- getModel("PLIERpriors", load=TRUE)
```

You can directly access the GSEA outputs for each RAV using `gsea`.
...
Find the RAVs annotated with the keyword-containing pathways. Without the `k` 
argument, the data frame containing the number of RAVs (`Freq` column) with 
the different numbers of keyword-containing, enriched pathways 
...
Enriched pathways of the given RAVmodel, ordered by NES. If `both=TRUE`, both 
the top and bottom enriched pathways will be printed.
